...
export const _assetList = [...Array(20)].map((_, index) => ({

    id: _mock.id(index),

    category: _mock.fullName(index),

    processor: _mock.fullName(index),

    productCompany: _mock.fullName(index),

    ram: '8',

    workingStatus: _mock.fullName(index),

    status:

    (index % 2 && 'active') || (index % 3 && 'repair')  || 'approved',
