...
// types
import { IUserItem, IUserProfileFriend } from 'src/types/user';
// _mock
...
function FriendCard({ friend }: FriendCardProps) {
  const { firstName, role, avatarUrl } = friend;

  const popover = usePopover();

  const handleDelete = () => {
    popover.onClose();
    console.info('DELETE', firstName);
  };

  const handleEdit = () => {
    popover.onClose();
    console.info('EDIT', firstName);
  };
  ...
  return (
    <>
      ...
      >
        <Avatar alt={firstName} src={avatarUrl} sx={{ width: 64, height: 64, mb: 3 }} />
...
function applyFilter({ inputData, query }: { inputData: IUserItem[]; query: string }) {
  if (query) {
    return inputData.filter(
      (friend) => friend.firstName.toLowerCase().indexOf(query.toLowerCase()) !== -1
    );
  }
