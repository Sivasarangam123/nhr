...
export default function ProfileFollowers({ followers }: Props) {
  const _mockFollowed = followers.slice(1, 2).map((i) => i.id);
...
function FollowerItem({ follower, selected, onSelected }: FollowerItemProps) {
  const { firstName, country, avatarUrl } = follower;
  ...
  return (
    ...
    >
      <Avatar alt={firstName} src={avatarUrl} sx={{ width: 48, height: 48, mr: 2 }} />
